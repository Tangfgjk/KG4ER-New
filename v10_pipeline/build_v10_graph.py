...
def require_or_copy_matrix(
    ...
    raise FileNotFoundError(
        ...
        f"or generate it under the V10 directory first. Use --allow-existing-state only for debugging."
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build V10 ER graph files from regenerated cognitive-state files.")
...
def main() -> None:
    ...
    # Keep old-cos recommendation scores for controlled comparison without mixing them into the main V10 graph.
    old_cos_dir = output_dir.parent / "v10_rec_legacy_cos"
    ...
    write_json(output_dir / "v10_graph_manifest.json", manifest)
    print(f"created V10 graph files: {output_dir}")
