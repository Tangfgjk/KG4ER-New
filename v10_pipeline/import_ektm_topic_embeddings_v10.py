...
from common import output_data_root, read_entity_dict, read_json, read_q_matrix, v10_dataset_dir, write_json
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import EKTM_mirt TopicRNNModel topic_v exercise embeddings into V10 semantic_kg_features."
    )
...
def main() -> None:
    args = parse_args()
    graph_dir = v10_dataset_dir(args.dataset, args.output_data_root)
    ...
    if missing_ex or missing_kc:
        raise ValueError(
            "V10 graph entities do not align with Q.txt: "
    ...
    if exercise_embeddings.shape[0] != len(exercise_ids):
        raise ValueError(
            "EKTM_mirt topic_v row count must match V10 exercise count: "
    ...
    manifest: dict[str, Any] = {
        ...
        "notes": (
            "Strict V10 uses EKTM_mirt TopicRNNModel topic_v for exercise text embeddings. "
