...
from common import output_data_root, v10_dataset_dir, write_json
...
def load_pykt_model(pykt_root: Path, checkpoint_dir: Path, device: str, checkpoint_file: str | None = None):
    ...
    if model_name != "dkt":
        raise ValueError(f"V10 seq exporter expects pyKT DKT, got model_name={model_name!r}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export V10 stu2know_seq.json from a trained pyKT DKT checkpoint.")
...
def main() -> None:
    ...
    # Keep the root graph file in sync for build_v10_graph.py.
