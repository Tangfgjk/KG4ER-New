...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from the best checkpoint into V10 front files."
        )
    )
    ...
    parser.add_argument("--a-file", type=Path, default=None, help="MIRT a parameter csv. Defaults to v10/mirt/outputs/a_param_*.csv.")
    parser.add_argument("--b-file", type=Path, default=None, help="MIRT b parameter csv. Defaults to v10/mirt/outputs/b_param_*.csv.")
...
def load_mirt_params(v10_dir: Path, a_file: Path | None, b_file: Path | None, exercise_count: int, concept_count: int) -> tuple[torch.Tensor, torch.Tensor, dict[str, Any]]:
...
def graph_uids_from_manifest(dataset: str, v10_dir: Path, learner_count: int) -> list[str]:
    manifest = read_json(v10_dir / "mirt" / "inputs" / "mirt_input_manifest.json")
    ...
    raise ValueError(f"Cannot recover graph uid alignment from {v10_dir / 'mirt/inputs/mirt_input_manifest.json'}")


def load_ordered_interactions(args: argparse.Namespace, v10_dir: Path, learner_count: int, exercise_count: int) -> tuple[pd.DataFrame, dict[str, Any]]:
    if args.interaction_file is not None:
        ...
        df = pd.read_csv(v10_dir / "mirt" / "inputs" / "all.csv")
        source_info = {
            "source": "mirt_all",
            "path": v10_dir / "mirt" / "inputs" / "all.csv",
        ...
        graph_uids = graph_uids_from_manifest(args.dataset, v10_dir, learner_count)
...
def export_topic_and_mastery(
    ...
    learner_count = max(learner_count, entity_count(read_entity_dict(v10_dataset_dir(args.dataset, args.output_data_root) / "entities.dict"), "uid"))
...
def write_outputs(
    ...
    if not args.force:
        targets = [
            ...
            v10_dir / "stu2know_mastery.json",
            text_dir / "exercise_text_embeddings.npy",
        ]
        existing = [path for path in targets if path.exists()]
        if existing:
            raise FileExistsError(f"V10 EKTM export targets already exist. Use --force to overwrite: {existing[:5]}")
...
def main() -> None:
    ...
    q_matrix = read_q_matrix(v10_dir / "Q.txt")
    ...
    a_tensor, b_tensor, mirt_report = load_mirt_params(v10_dir, args.a_file, args.b_file, exercise_count, concept_count)
    interactions, source_report = load_ordered_interactions(args, v10_dir, learner_count, exercise_count)
    ...
    write_outputs(args, v10_dir, topic_matrix, concept_matrix, mastery, checkpoint_report, source_report, mirt_report)
    print(
        json.dumps(
            {
                "dataset": args.dataset,
                ...
                "output_dir": str(v10_dir / "ektm_mirt" / "exports"),
            },
            ensure_ascii=False,
            indent=2,
        )
