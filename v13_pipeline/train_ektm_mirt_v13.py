...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a v13 EKTM_mirt-style model on ER-aligned learners and export "
...
def ensure_v13_core_files(source_graph_dir: Path, v13_dir: Path) -> None:
...
def load_mirt_params(v13_dir: Path, exercise_count: int, concept_count: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, dict[str, Any]]:
    ...
    if a.shape[1] != concept_count:
        raise ValueError(
            f"v13 no-Q MIRT latent_dim should equal concept_count for EKTM-style training; "
...
def graph_uid_map(dataset: str, v13_dir: Path, learner_count: int) -> tuple[dict[str, int], dict[str, Any]]:
    manifest_path = v13_dir / "mirt" / "inputs" / "mirt_input_manifest.json"
...
def load_ordered_interactions(dataset: str, source_dir: Path, v13_dir: Path, learner_count: int, exercise_count: int) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    uid_map, uid_report = graph_uid_map(dataset, v13_dir, learner_count)
    work = raw.copy()
...
def batch_loss(model: v13EKTMmirt, batch: dict[str, torch.Tensor], device: torch.device, mastery_weight: float) -> tuple[torch.Tensor, dict[str, float]]:
...
@torch.no_grad()
def export_mastery(model: v13EKTMmirt, dataset: LearnerSequenceDataset, learner_count: int, concept_count: int, device: torch.device) -> np.ndarray:
...
def save_exports(
    ...
    write_json(
        text_dir / "text_embedding_manifest.json",
        {
            ...
            "model": "v13_EKTM_mirt_style_BiGRU_topic_encoder",
            "source": "topic_v_from_train_ektm_mirt_v13",
    ...
    write_json(
        ...
        {
            "dataset": args.dataset,
            "model": "v13_EKTM_mirt_style",
            "checkpoint": v13_dir / "ektm_mirt" / "best.pt",
...
def main() -> None:
    ...
    ensure_v13_core_files(source_graph_dir, v13_dir)
    ...
    q_matrix = read_q_matrix(v13_dir / "Q.txt").astype(np.float32)
    ...
    a_param, b_param, _theta, mirt_report = load_mirt_params(v13_dir, exercise_count, concept_count)
    ...
    interactions, interaction_report = load_ordered_interactions(args.dataset, source_dir, v13_dir, learner_count, exercise_count)
    ...
    save_exports(args, v13_dir, model, seq_dataset, learner_count, concept_count, device, report)
    write_json(output_dir / "ektm_train_manifest.json", report)
    print(f"saved v13 EKTM_mirt-style checkpoint and exports: {output_dir}")
