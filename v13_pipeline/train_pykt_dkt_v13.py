...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and export v13 stu2know_seq.json."
        )
    )
    ...
    parser.add_argument(
        "--skip-train",
        action="store_true",
        help="Only export stu2know_seq.json from an existing v13 patched pyKT checkpoint.",
    )
...
def copy_pykt(pykt_root: Path, work_root: Path, force: bool) -> Path:
    target = work_root / "pykt-toolkit-v13"
    if target.exists():
        if force:
...
def patch_pykt_for_pkc(pykt_copy: Path) -> dict[str, Any]:
    ...
    new_train = (
        ...
        "        y = torch.masked_select(ys[0], sm)\n"
        "        if model_name == \"dkt\":\n"
        "            # v13 PKC-DKT: only supervise the next observed concept positions as positive labels.\n"
    ...
    # pyKT selects the best checkpoint by validation AUC. v13 PKC-DKT sets all
    ...
    train_text = train_text.replace(
        "        if i - best_epoch >= 10:\n            break",
        "        # v13 PKC-DKT uses the last checkpoint for seq export, so do not early-stop by AUC.\n"
    ...
    new_eval = (
        "# v13 PKC-DKT evaluation target: next observed concept positions are positive labels.\n"
    ...
    new_auc_block = (
        ...
        "            # v13 PKC-DKT uses positive-only next-concept labels; ROC-AUC is undefined.\n"
...
def export_seq_with_checkpoint(pykt_copy: Path, checkpoint_dir: Path, test_sequences: Path, output_file: Path, expected_students: int, expected_concepts: int, device: str) -> dict[str, Any]:
    ...
    from export_seq_from_dkt_v13 import export_seq, load_pykt_model
    ...
    summary["checkpoint_policy"] = "prefer_last_model_for_v13_pkc_dkt"
    return summary
...
def main() -> None:
    ...
    dkt_dir = v13_dir / "pykt_dkt" / "dkt_concept"
    work_root = v13_dir / "pykt_dkt" / "work"
    checkpoint_manifest = v13_dir / "pykt_dkt" / "checkpoint_manifest.json"

    if args.dataset == "Eedi" and not (source_graph_dir / "train_sequences.csv").exists():
        mirt_manifest_path = v13_dir / "mirt" / "inputs" / "mirt_input_manifest.json"
        if not mirt_manifest_path.exists():
            raise FileNotFoundError(
                f"missing {mirt_manifest_path}. Run prepare_mirt_inputs_v13.py before train_pykt_dkt_v13.py."
            )
        ...
        source_graph_dir, eedi_sequence_manifest = prepare_eedi_sequence_interactions_as_standard_files(
            source_dir,
            v13_dir / "pykt_dkt" / "source_sequences",
            learner_count,
            args.force,
        )
    else:
    ...
    dataset_name = f"{args.dataset}_v13_pkc_dkt"
    ...
    if args.skip_train:
        ...
        write_json(
            checkpoint_manifest,
            {
                ...
                "export_checkpoint_policy": "prefer qid_last_model.ckpt because v13 PKC-DKT validation AUC is not informative",
                "params": params,
    ...
    output_file = v13_dir / "pykt_dkt" / "stu2know_seq.json"
    ...
    (v13_dir / "stu2know_seq.json").write_text(output_file.read_text(encoding="utf-8"), encoding="utf-8")
    write_json(
        v13_dir / "pykt_dkt" / "seq_export_manifest.json",
        {
            "dataset": args.dataset,
            "source": "v13 patched pyKT DKT; next observed concept labels are set to 1",
            "checkpoint_dir": checkpoint_dir,
