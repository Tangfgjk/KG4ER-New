...
from pkc_dkt_model_v13 import PKCDKT
from prepare_mirt_inputs_v13 import eedi_valid_question_length
...
class PKCSequenceDataset(Dataset):
    def __init__(self, csv_path: Path, concept_count: int) -> None:
        ...
        for row in rows:
            ...
            if inputs_c:
                sample_targets = targets[: len(inputs_c)]
                self.positive_labels += int(sample_targets.sum())
                self.total_labels += int(sample_targets.size)
                self.samples.append((inputs_c, inputs_r, sample_targets))
    ...
    def auto_positive_weight(self, max_weight: float = 20.0) -> float:
        positives = max(1, int(self.positive_labels))
        negatives = max(1, int(self.total_labels - self.positive_labels))
        return float(min(max_weight, negatives / positives))
...
def run_epoch(
    ...
    for concepts, responses, targets, mask in tqdm(loader, disable=not training):
        ...
        mask = mask.to(device)
        logits = model(concepts, responses)
        weights = torch.ones_like(targets[mask])
        weights = torch.where(targets[mask] > 0.5, weights * float(positive_weight), weights)
        loss = F.binary_cross_entropy_with_logits(logits[mask], targets[mask], weight=weights)
        if training:
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def export_test_sequence_predictions(
    ...
    for row in rows:
        ...
        logits = model(c, r)[0, len(inputs_c) - 1]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train V13 next-KC DKT and export stu2know_seq.json. "
            "The target at each step is the multi-hot knowledge-concept vector of the next exercise."
        )
    )
    ...
    parser.add_argument(
        "--positive-weight",
        default="auto",
        help="Positive-label weight for multi-label BCE. Use 'auto' or a positive float.",
    )
    parser.add_argument("--max-auto-positive-weight", type=float, default=20.0)
...
def main() -> None:
    ...
    if args.export_only:
        ...
        if str(args.positive_weight).lower() == "auto":
            positive_weight = dataset.auto_positive_weight(max_weight=args.max_auto_positive_weight)
        else:
            positive_weight = float(args.positive_weight)
            if positive_weight <= 0:
                raise ValueError("--positive-weight must be positive or 'auto'")
        ...
        print(
            ...
            f"positive_labels={dataset.positive_labels}",
            f"total_labels={dataset.total_labels}",
            f"positive_weight={positive_weight:.4f}",
        )
        for epoch in range(args.epochs):
            train_loss = run_epoch(model, train_loader, device, optimizer, positive_weight)
            with torch.no_grad():
                valid_loss = run_epoch(model, valid_loader, device, None, positive_weight)
    ...
    (v13_dir / "stu2know_seq.json").write_text((output_dir / "stu2know_seq.json").read_text(encoding="utf-8"), encoding="utf-8")
    write_json(
        output_dir / "pkc_dkt_manifest.json",
        {
            "dataset": args.dataset,
            "source": "v13 PKC-DKT trained locally for next-knowledge-concept occurrence prediction",
            ...
            "positive_weight": best_payload.get("positive_weight", positive_weight),
