...
embedding_path = f"./models/algebra2005/RotatE_tmp"
type = embedding_path.split('/')[-1]
...
def RotatE(head, relation, tail, gamma=12.0, epsilon=2.0, hidden_dim=500):
    ...
    phase_relation = relation / ((gamma + epsilon) / hidden_dim / pi)
...
if not os.path.exists(f'{type}_uid_ex_scores.pkl'):  # 如果文件不存在
    for uid in uid_mlkc_dict_keys_list:
        ...
        for i in range(len(uid_mlkc_keys)):
            ...
            s_mlkc_list.append(RotatE_emb(kc_embedding, mlkc_embedding))
            s_pkc_list.append(RotatE_emb(kc_embedding, pkc_embedding))
        for qid in tqdm(range(len(Q))):
            e = torch.from_numpy(dict_entity_embedding['ex' + str(qid)])
            fr1 = 0.0
            fr2 = 0.0

            for s_mlkc in s_mlkc_list:
                fr1 += RotatE(s_mlkc, rec_embedding, e)

            for s_pkc in s_pkc_list:
                fr1 += RotatE(s_pkc, rec_embedding, e)
            ...
            s_efr = RotatE_emb(ej_embedding, efr_embedding)
            fr2 = RotatE(s_efr, rec_embedding, e)
    ...
    with open(f'{type}_uid_ex_scores.pkl', 'wb') as f:
        pickle.dump(uid_ex_scores, f)
else:
    with open(f'{type}_uid_ex_scores.pkl', 'rb') as f:
