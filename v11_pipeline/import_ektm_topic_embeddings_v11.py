...
from common import output_data_root, read_entity_dict, read_json, read_q_matrix, v11_dataset_dir, write_json
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import EKTM_mirt TopicRNNModel topic_v exercise embeddings into V11 semantic_kg_features."
    )
...
def main() -> None:
    ...
    if missing_ex or missing_kc:
        raise ValueError(
            "V11 graph entities do not align with Q.txt: "
    ...
    if exercise_embeddings.shape[0] != len(exercise_ids):
        raise ValueError(
            "EKTM_mirt topic_v row count must match V11 exercise count: "
    ...
    manifest: dict[str, Any] = {
        ...
        "notes": (
            "Strict V11 uses EKTM_mirt TopicRNNModel topic_v for exercise text embeddings. "
