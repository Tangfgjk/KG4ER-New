...
def require_or_copy_matrix(
    ...
    raise FileNotFoundError(
        ...
        f"or generate it under the V11 directory first. Use --allow-existing-state only for debugging."
    )
...
def main() -> None:
    ...
    # Keep old-cos recommendation scores for controlled comparison without mixing them into the main V11 graph.
    old_cos_dir = output_dir.parent / "v11_rec_legacy_cos"
    if args.sequence_term == "one_minus_cos_sq":
        ...
        write_json(
            old_cos_dir / "v11_graph_manifest.json",
            {
                "dataset": args.dataset,
                "version": "v11_front_files_legacy_cos",
                "source_graph_dir": source_graph_dir,
                "output_dir": old_cos_dir,
                "base_v11_dir": output_dir,
                "state_sources": {
                ...
                "forgetting": {
                    "stu2ex_forget": "copied from V11 average exercise forgetting",
    ...
    write_json(output_dir / "v11_graph_manifest.json", manifest)
    print(f"created V11 graph files: {output_dir}")
