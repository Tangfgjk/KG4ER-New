...
def validate_dataset(dataset: str, data_root: Path) -> dict[str, Any]:
    graph_dir = v11_dataset_dir(dataset, data_root)
    errors: list[str] = []
    warnings: list[str] = []
    required = [
        "entities.dict",
    ...
    if feature_status["text_manifest"]:
        ...
        if "bge" in model_name or "sentence" in model_name:
            errors.append(
                ...
                "strict V11 should use EKTM_mirt TopicRNNModel topic_v embeddings"
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate V11 front files after regeneration.")
...
def main() -> None:
    ...
    output_file = args.output_file or (args.output_data_root / "v11_validation_report.json")
