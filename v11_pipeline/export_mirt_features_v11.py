...
def build_exercise_features(
    ...
    for idx in range(exercise_count):
        ...
        features[f"ex{idx}"] = {
            ...
            "feature_source": "v11_graph_subset_EduCDM_modified_no_q_MIRT",
        }
    return features
...
def build_learner_features(
    ...
    if mastery.ndim != 2 or mastery.shape[0] != learner_count:
        raise ValueError(
            "mastery matrix must align with V11 graph learners: "
    ...
    for idx in range(learner_count):
        ...
        features[f"uid{idx}"] = {
            ...
            "feature_source": "v11_MIRT_theta_plus_V11_mastery",
        }
    return features
...
def copy_text_features(source_feature_dir: Path | None, output_feature_dir: Path) -> None:
    if source_feature_dir is None:
        raise FileNotFoundError(
            ...
            "V11 can reuse concept/exercise text metadata, but text embeddings must be imported "
...
def require_ektm_text_embeddings(output_feature_dir: Path) -> None:
    ...
    if not manifest_path.exists():
        raise FileNotFoundError(
            f"Missing {manifest_path}. Strict V11 no longer accepts BGE text embeddings. "
            "Run v11_pipeline/import_ektm_topic_embeddings_v11.py first."
        )
    ...
    if "bge" in model_name or "sentence" in model_name:
        raise ValueError(
            f"Legacy BGE/SentenceTransformer text embeddings are not allowed in V11: {manifest_path}. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export V11 MIRT-derived feature files for SemanticConvE.")
...
def main() -> None:
    ...
    input_dir = args.input_dir or (v11_dir / "mirt" / "inputs")
    mirt_output_dir = args.mirt_output_dir or (v11_dir / "mirt" / "outputs")
    output_feature_dir = args.output_feature_dir or (v11_dir / "semantic_kg_features")
    mastery_file = args.mastery_file or (v11_dir / "stu2know_mastery.json")
    ...
    write_json(
        output_feature_dir / "feature_generation_manifest.json",
        {
            ...
            "feature_version": "v11_graph_subset_mirt_features",
            ...
            "notes": (
                ...
                "from V11 MIRT and V11 mastery. Text embeddings must come from EKTM_mirt TopicRNNModel topic_v; "
                "legacy BGE/SentenceTransformer embeddings are rejected."
            ),
        },
    )
    print(f"saved V11 semantic feature files: {output_feature_dir}")
